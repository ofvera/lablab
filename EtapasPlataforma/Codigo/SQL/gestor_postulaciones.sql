SELECT user.id as User_ID, job_manager_item.id as Item_ID, CONCAT(user.firstname," ",user.lastname) as Nombre, user.username as RUT, program.name as Programa,
...
WHERE (user.deletedAt is null) AND (user.enabled = 1) AND (job_manager_item_history.created >= '2020-01-01');